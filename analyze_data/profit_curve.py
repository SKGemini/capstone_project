...
def profit_curve_main(X,y,cost_benefit):
...
def oversample(X, y, tp):
    ...
    Returns
    -------
    X_oversampled : ndarray - 2D
    y_oversampled : ndarray - 1D
    """
    if tp < np.mean(y):
        return X, y
    neg_count, pos_count, X_pos, X_neg, y_pos, y_neg = div_count_pos_neg(X, y)
...
def sampling_main(model, X, y, cost_benefit, range_params=(0.35, 0.65, 0.05)):
